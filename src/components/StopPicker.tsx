...
export const StopPicker = () => {
    ...
    const StopMarker = ({ stop }: { stop: IStop }) => {
        ...
        return (
            <Marker position={position}>
                <Popup>
                    <a href={`/stop/${SOLMUTUNNU}?lat=${position[0]}&lng=${position[1]}`}>{LYHYTTUNNU} - {NIMI1} {NIMI2}</a>
                </Popup>
            </Marker>
        )
    }
    ...
    return <div>
        {loading && <div className='map-loading'><span>Loading</span></div>}
        ...
        >
            ...
            />
            {fields.map(el => <StopMarker key={el.properties.SOLMUTUNNU} stop={el} />)}
        </MapContainer>
    </div>
}
